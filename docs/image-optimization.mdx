...
```ts
// pyra.config.ts
import { defineConfig } from '@pyra/core';
...
The React adapter exports an `<Image>` component that builds the right HTML for responsive images. Import it from `@pyra/adapter-react`:
