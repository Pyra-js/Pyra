...
```ts
// pyra.config.ts
import { defineConfig } from '@pyra-js/core';
...
The React adapter exports an `<Image>` component that builds the right HTML for responsive images. Import it from `@pyra-js/adapter-react`:
