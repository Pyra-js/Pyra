...
Shared types, configuration loader, logger, and network utilities for the Pyra.js framework. This package is an internal dependency, it is consumed by `@pyra-js/core`, `@pyra-js/adapter-react`, and `@pyra-js/cli`. You do not need to install it directly in an application.
...
```ts
import { loadConfig, resolveConfig, findConfigFile } from '@pyra-js/shared';
